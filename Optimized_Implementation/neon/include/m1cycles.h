...
#undef F
    ...
    if (c1 != CONFIG_COUNT)
    {
        /* Patched: different M1/M2 variants report a different total counter
         * count (e.g. 10). Warn but proceed -- the core-cycle counter is read
         * from g_counters[2] regardless. */
        printf("note: counter count = %d (expected %d); proceeding\n", c1, CONFIG_COUNT);
    }

    const unsigned c2 = kpc_get_config_count(KPC_MASK);
    if (c2 != 6)
    {
        printf("note: config count = %d (expected 6); proceeding\n", c2);
    }
